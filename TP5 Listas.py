...
ranking = sorted(puntajes, reverse = True) # Generamos la lista nueva con sorted de la lista puntajes de mayor a menor, es decir el ranking.
print()
print('''>> La lista ordenada de mayor a menor, o bien, el "RANKING" es el siguiente:''')
for i in range(len(ranking)): # Utilizo esta condicion para mostrar de una forma más estética la lista.
    if i == len(ranking) - 1:
        print(f"{ranking[i]}.")
    else:
        print(f"{ranking[i]},", end = " ")

print()
for i in range(len(ranking)): # Ciclo para buscar el puntaje "990" en la lista del Ranking.
    if ranking[i] == 990:
        pos_990 = i+1
        print(f'''>> El puntaje "990" se encuentra en la posición {pos_990} del ranking.''')
        break